      ...
      <div className="flex-1 bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 text-left">
                    <div className="flex items-center justify-between">
                      <h3 className={`font-medium truncate ${unread > 0 ? 'text-slate-900 font-semibold' : 'text-slate-900'}`}>
                        {contact.name}
                      </h3>
                      {lastMsg && (
                    ...
                    <div className="flex items-center justify-between mt-0.5">
                      <p className={`text-sm truncate ${
                        ...
                        {lastMsg?.attachments && JSON.parse(lastMsg.attachments || '[]').length > 0
                          ? '📎 Photo'
                          : (lastMsg?.content || 'No messages yet')}
                      </p>
                      {unread > 0 && (
                        <span className="w-5 h-5 bg-emerald-500 text-white text-xs rounded-full flex items-center justify-center ml-2 flex-shrink-0">
                          {unread}
                        </span>
                      )}
                    </div>
        ...
        {selectedUser ? (
          <div className="flex-1 flex flex-col">
              ...
              {groupedMessages.map((group, gi) => (
                <div key={gi} className="mb-4">
                  <div className="flex items-center justify-center mb-3">
                    <span className="px-3 py-1 bg-slate-200/80 rounded-full text-xs text-slate-500 font-medium">
                      {group.date}
                    </span>
                  </div>
                  {group.msgs.map((msg) => {
            ...
            {/* Typing indicator */}
            {typingUsers.size > 0 && selectedUser && typingUsers.has(selectedUser) && (
            ...
            <div className="p-4 border-t border-slate-100 bg-white">
              <div className="flex items-center gap-2">
                ...
                <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="p-2 hover:bg-slate-100 rounded-lg transition disabled:opacity-50 flex-shrink-0">
                  <Paperclip size={20} className="text-slate-500" />
                </button>
                <button onClick={() => cameraInputRef.current?.click()} disabled={uploading} className="p-2 hover:bg-slate-100 rounded-lg transition disabled:opacity-50 flex-shrink-0">
                  <Camera size={20} className="text-slate-500" />
                </button>
                <button
                  onClick={toggleRecording}
                  disabled={uploading}
                  className={`p-2 rounded-lg transition disabled:opacity-50 flex-shrink-0 ${recording ? 'bg-red-100 animate-pulse' : 'hover:bg-slate-100'}`}
                >
                ...
                <input
                  ...
                  className="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500 text-sm"
                />
                <button
                  onClick={handleSend}
                  disabled={(!newMessage.trim() && attachments.length === 0) || uploading}
                  className="p-2.5 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 disabled:opacity-50 transition flex-shrink-0"
                >
