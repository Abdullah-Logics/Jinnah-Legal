...
import { Bot, Send, Loader, Plus, MessageSquare, Trash2, FileDown, PanelLeftClose, PanelLeft } from 'lucide-react';
  ...
  const handleSend = async (text?: string) => {
    ...
    try {
      const res = await fetch(`${API}/api/ai/chat`, {
        ...
        body: JSON.stringify({ message: msg, history: messages.map(m => ({ role: m.role === 'ai' ? 'assistant' : 'user', content: m.content })), sessionId: sid }),
      });
      const data = await res.json();
        ...
        <div className="bg-gradient-to-r from-emerald-600 to-emerald-800 rounded-2xl p-6 text-white mb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-emerald-200">Powered by Gemini — I can help with questions, draft documents, manage cases & more</p>
            </div>
          </div>
        </div>
