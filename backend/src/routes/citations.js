...
citationsRouter.get('/:id', asyncHandler(async (req, res) => {
  const c = await queryOne('SELECT id, title, citation, court, year, parties, category, description, full_text, pdf_url, metadata, relevant_statutes, keywords, created_at FROM citations WHERE id=?', [req.params.id]);
...
citationsRouter.get('/suggest/top', asyncHandler(async (req, res) => {
  ...
  const rows = await query(
     `SELECT id, title, citation, court, year, parties, category, description, full_text, pdf_url, metadata, relevant_statutes, keywords, created_at FROM citations
